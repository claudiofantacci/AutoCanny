...
void AutoThreshold(const cv::Mat& GX, const cv::Mat& GY, double& high, double& low,
                   const double percent_nonedge_pixels = 0.8, const double highlow_threshold_ratio = 0.4)
{
    cv::Mat      mag_gradient;
    cv::Mat      mag_hist;
    ...
    const double percentile   = percent_nonedge_pixels * GX.cols * GX.rows;
...
void AutoCanny(const cv::Mat& src, cv::Mat& dst,
               const double sigma, const double percent_nonedge_pixels, const double highlow_threshold_ratio)
{
    cv::Mat GX;
    cv::Mat GY;
    ...
    ImgGradient(src, GX, GY, sigma);

    AutoThreshold(GX, GY, high_threshold, low_threshold, percent_nonedge_pixels, highlow_threshold_ratio);
